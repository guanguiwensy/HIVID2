...
#	print OUTSH "date\n";
	...
	while(my $LIST=<LIST>){
		...
		print OUTSH "#!/bin/bash\n#PBS -N trimmomatic.sh\n#PBS -l nodes=1:ppn=5\n#PBS –l walltime=100:00:00\n#PBS –l mem=10G\n#PBS -q batch\n#PBS -V\ncd \$PBS_O_WORKDIR\n";
	    print OUTSH "date\n";
		print OUTSH "java -jar $bin/Trimmomatic-0.39/trimmomatic-pigz.jar PE -threads $thread -phred33 $a[-2] $a[-1] $step2dir/$sample_name/$base_name1.trimmo.paired.gz $step2dir/$sample_name/$base_name1.trimmo.unpaired.gz $step2dir/$sample_name/$base_name2.trimmo.paired.gz $step2dir/$sample_name/$base_name2.trimmo.unpaired.gz ILLUMINACLIP:$bin/Trimmomatic-0.39/adapters/TruSeq3-PE.fa:2:30:10 LEADING:3 TRAILING:3 SLIDINGWINDOW:4:15 MINLEN:60\n";
		print OUTSH "java -jar $bin/Trimmomatic-0.39/trimmomatic-pigz.jar PE -threads $thread -phred33 $a[-2] $a[-1] $step2dir/$sample_name/$base_name1.trimmo.paired1.gz $step2dir/$sample_name/$base_name1.trimmo.unpaired1.gz $step2dir/$sample_name/$base_name2.trimmo.paired1.gz $step2dir/$sample_name/$base_name2.trimmo.unpaired1.gz ILLUMINACLIP:$bin/Trimmomatic-0.39/adapters/TruSeq3-PE.fa:2:30:10 LEADING:0 TRAILING:0 SLIDINGWINDOW:4:0\n";		print OUTSH "date\n";
##		chdir $step2dir;
